       ...
       int final)        /* wykonaj program, policz jego czasy, sprawdz, czy to nie jest ostatni program */
{
    ...
    if (start == -1)            /* zapamietaj wartosci poczatkowe pomiaru czasu*/
    {
        printf("times error");
        exit(-1);
    }
    ...
    if (end == -1)        /* zapamietaj wartosci koncowe */
    {
        printf("times error");
        exit(-1);
    }
...
display_times(clock_t real, struct tms *tmsstart, struct tms *tmsend) {
    ...
    if (clktck == 0){
//        if(getrusage(clktck, ru_inblock) > 1){
//            printf("used more than 1 time");
//            exit(-1);
//        }/* sprawdz, czy stan zmiennej odczytywany pierwszy raz */
